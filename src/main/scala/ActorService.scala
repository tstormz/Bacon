...
class ActorService(actors: Session, executionContext: ExecutionContext) {
    ...
    private def getFilmography(actorId: String): List[Picture] = {
        ...
        for (row: Row <- results.all()) {
            ...
            pictureMatches ::= Picture(id, title, year)
        }
        pictureMatches
    }
